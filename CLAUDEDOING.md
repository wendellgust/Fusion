...
**Changed**:
- `packages/player/src/stores/queueStore.ts`: added `moveFailedCurrentItemToCacheEnd(cacheWindowSize)` method
- `packages/player/src/hooks/useStreamResolution.ts`: implemented 5-track cache sliding prefetch window, background retries for tracks 2..5, and error handling repositioning failed 1st track to 5th cache end
- `packages/player/src/components/SoundProvider.tsx`: integrated `handleCurrentTrackFailure` into `handleError`
