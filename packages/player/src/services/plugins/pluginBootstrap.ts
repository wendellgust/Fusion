...
export const hydratePluginsFromRegistry = async (): Promise<void> => {
  ...
  try {
    ...
    for (const entry of entries) {
      if (!entry?.path || !(await isManagedPath(entry.path))) {
        continue;
      }
    ...
    console.warn('Plugin hydration warning:', err);
  } finally {
  ...
  void checkAndUpdatePlugins().catch(() => {});
};
