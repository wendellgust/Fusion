...
export const SoundProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const syncMediaSessionHandlers = useCallback(() => {
    ...
    // CRITICAL FOR IOS:
    // Unregister seek handlers (null) so iOS MPRemoteCommandCenter disables
    // SkipForwardCommand/SkipBackwardCommand (+10s / -10s) and enables
    // NextTrackCommand/PreviousTrackCommand (>>| and |<<) to change entire songs.
